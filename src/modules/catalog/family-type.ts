/**
 * catalog — kategória átvétele a MODELLCSALÁDON belül (F2.1-utó-21,
 * 2026-08-19; a catalog modulba emelve 2026-08-22).
 *
 ...
 * deszka viszont kategória NÉLKÜL maradt.
 *
 * MIÉRT ITT, ÉS NEM A `tools/catalog-watch` ALATT: a szabály eddig a figyelő
 * mellett élt, ezért CSAK a parancssori tömeges jóváhagyó látta — a moderációs
 * FELÜLET nem. A moderátornak így minden régebbi évjáratnál magának kellett
 * kitalálnia a kategóriát, holott a modellcsalád már be volt sorolva. Mérve
 * (2026-08-22): a 74 kategória nélküli Starboard-jelöltből 37 örökölhető.
 *
 * A `tools` importálhat a `src`-ből (a `types.ts` és a `gear.ts` is így megy),
 * fordítva viszont nem — ezért a közös logika ide került, és a figyelő innen
 * veszi (`tools/catalog-watch/family-type.ts` már csak továbbadja).
 *
